...
class FrameAdministrarCarrera(Frame):
    ...
    def _frame_superior(self, frame: Frame):
        label_info = Label(
            frame,
            text=f"{ICON_CARRERA} Administador de Carreras",
            bootstyle="info",
    ...
    def _frame_tabla(self, frame: Frame):
        ...
        self.tabla_carrera.pack(side=TOP, fill=BOTH, padx=1, pady=1, expand=TRUE)
    ...
    def _frame_formulario(self, frame: Frame):
        ...
        # Plan
        lbl_plan = Label(frame_campos, text="📅 Plan:", anchor=W, font=("Helvetica", 10, "bold"))
        lbl_plan.grid(column=0, row=2, sticky=W, padx=5, pady=(5, 2))

        self.entry_plan = Entry(
            frame_campos, textvariable=self.var_plan, justify=CENTER, bootstyle="info"
        )
        self.entry_plan.grid(column=0, row=3, padx=5, pady=(0, 8), sticky=EW)
        ...
        lbl_credito.grid(column=1, row=2, padx=5, pady=(5, 2), sticky=W)

        self.entry_credito = Entry(
            frame_campos, textvariable=self.var_credito, justify=RIGHT, bootstyle="info"
        )
        self.entry_credito.grid(column=1, row=3, sticky=EW, padx=5, pady=(0, 8))
        self.map_widgets['entry_credito'] = self.entry_credito

        # Nombre (ocupa todo el ancho)
        lbl_nombre = Label(
            frame_campos, text="📚 Nombre de la Carrera:", anchor=W, font=("Helvetica", 10, "bold")
        )
        ...
        lbl_modalidad = Label(
            frame_campos, text="🏫 Modalidad:", anchor=W, font=("Helvetica", 10, "bold")
        )
        lbl_modalidad.grid(column=0, row=6, padx=5, pady=(5, 2), sticky=W, columnspan=2)
        ...
        self.cbx_modalidad.grid(column=0, row=7, pady=(0, 10), sticky=EW, columnspan=2, padx=5)
        ...
        frame_navegacion.grid(column=0, row=10, columnspan=2, sticky=EW, padx=5, pady=(5, 0))
